      ...
      c.auth_scheme_resolver = function(_service, operation)
         ...
         for _, scheme in ipairs(auth_trait or {}) do
            ...
            if scheme_id == "aws.auth#sigv4" or scheme_id == "aws.auth#sigv4a" then
               options[#options + 1] = { scheme_id = scheme_id, signer_properties = { signing_name = "awsssoportal", signing_region = c.region } }
            else
...
function C:getRoleCredentials(input, options)
   return self:invokeOperation(schemas.Service, schemas.GetRoleCredentials, input, options)
end

function C:listAccountRoles(input, options)
...
function C:logout(input, options)
