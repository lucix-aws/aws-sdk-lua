...
function M.new(cfg)
   local c = cfg or {}
   c.service_id = "AWSBCMDashboardsService"
      ...
      c.auth_scheme_resolver = function(_service, operation)
         ...
         for _, scheme in ipairs(auth_trait or {}) do
            ...
            if scheme_id == "aws.auth#sigv4" or scheme_id == "aws.auth#sigv4a" then
               options[#options + 1] = { scheme_id = scheme_id, signer_properties = { signing_name = "bcm-dashboards", signing_region = c.region } }
            else
...
function C:createScheduledReport(input, options)
...
function C:deleteScheduledReport(input, options)
   return self:invokeOperation(schemas.Service, schemas.DeleteScheduledReport, input, options)
end

function C:executeScheduledReport(input, options)
...
function C:getScheduledReport(input, options)
...
function C:listScheduledReports(input, options)
...
function C:updateScheduledReport(input, options)
