      ...
      c.auth_scheme_resolver = function(_service, operation)
         ...
         for _, scheme in ipairs(auth_trait or {}) do
            ...
            if scheme_id == "aws.auth#sigv4" or scheme_id == "aws.auth#sigv4a" then
               options[#options + 1] = { scheme_id = scheme_id, signer_properties = { signing_name = "s3-outposts", signing_region = c.region } }
            else
...
function C:listOutpostsWithS3(input, options)
   return self:invokeOperation(schemas.Service, schemas.ListOutpostsWithS3, input, options)
end

function C:listSharedEndpoints(input, options)
