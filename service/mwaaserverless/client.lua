...
function M.new(cfg)
   local c = cfg or {}
   c.service_id = "AmazonMWAAServerless"
      ...
      c.auth_scheme_resolver = function(_service, operation)
         ...
         for _, scheme in ipairs(auth_trait or {}) do
            ...
            if scheme_id == "aws.auth#sigv4" or scheme_id == "aws.auth#sigv4a" then
               options[#options + 1] = { scheme_id = scheme_id, signer_properties = { signing_name = "airflow-serverless", signing_region = c.region } }
            else
...
function C:getTaskInstance(input, options)
   return self:invokeOperation(schemas.Service, schemas.GetTaskInstance, input, options)
end
...
function C:listTaskInstances(input, options)
   return self:invokeOperation(schemas.Service, schemas.ListTaskInstances, input, options)
end
