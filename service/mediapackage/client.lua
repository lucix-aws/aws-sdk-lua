      ...
      c.auth_scheme_resolver = function(_service, operation)
         ...
         for _, scheme in ipairs(auth_trait or {}) do
            ...
            if scheme_id == "aws.auth#sigv4" or scheme_id == "aws.auth#sigv4a" then
               options[#options + 1] = { scheme_id = scheme_id, signer_properties = { signing_name = "mediapackage", signing_region = c.region } }
            else
...
function C:describeHarvestJob(input, options)
   return self:invokeOperation(schemas.Service, schemas.DescribeHarvestJob, input, options)
end

function C:describeOriginEndpoint(input, options)
...
function C:rotateChannelCredentials(input, options)
   return self:invokeOperation(schemas.Service, schemas.RotateChannelCredentials, input, options)
end

function C:rotateIngestEndpointCredentials(input, options)
