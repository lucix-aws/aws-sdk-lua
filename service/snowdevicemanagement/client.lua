...
function M.new(cfg)
   local c = cfg or {}
   c.service_id = "SnowDeviceManagement"
      ...
      c.auth_scheme_resolver = function(_service, operation)
         ...
         for _, scheme in ipairs(auth_trait or {}) do
            ...
            if scheme_id == "aws.auth#sigv4" or scheme_id == "aws.auth#sigv4a" then
               options[#options + 1] = { scheme_id = scheme_id, signer_properties = { signing_name = "snow-device-management", signing_region = c.region } }
            else
...
function C:cancelTask(input, options)
...
function C:describeDeviceEc2Instances(input, options)
...
function C:listDeviceResources(input, options)
