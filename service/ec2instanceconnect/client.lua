...
function M.new(cfg)
   local c = cfg or {}
   c.service_id = "AWSEC2InstanceConnectService"
      ...
      c.auth_scheme_resolver = function(_service, operation)
         ...
         for _, scheme in ipairs(auth_trait or {}) do
            ...
            if scheme_id == "aws.auth#sigv4" or scheme_id == "aws.auth#sigv4a" then
               options[#options + 1] = { scheme_id = scheme_id, signer_properties = { signing_name = "ec2-instance-connect", signing_region = c.region } }
            else
...
function C:sendSerialConsoleSSHPublicKey(input, options)
   return self:invokeOperation(schemas.Service, schemas.SendSerialConsoleSSHPublicKey, input, options)
end

function C:sendSSHPublicKey(input, options)
