      ...
      c.auth_scheme_resolver = function(_service, operation)
         ...
         for _, scheme in ipairs(auth_trait or {}) do
            ...
            if scheme_id == "aws.auth#sigv4" or scheme_id == "aws.auth#sigv4a" then
               options[#options + 1] = { scheme_id = scheme_id, signer_properties = { signing_name = "artifact", signing_region = c.region } }
            else
...
function C:listReportVersions(input, options)
   return self:invokeOperation(schemas.Service, schemas.ListReportVersions, input, options)
end

function C:putAccountSettings(input, options)
