...
function M.resolve_identity_resolver(cfg)
    ...
    local chain = require("aws.sdk.runtime.credentials.chain")
    local env_provider = require("aws.sdk.runtime.credentials.environment")
    ...
    if cfg.access_key_id and cfg.secret_access_key then
        local static = require("aws.sdk.runtime.credentials.static")
    ...
    if resolved.access_key_id and resolved.secret_access_key then
        local shared = require("aws.sdk.runtime.credentials.shared_config")
        providers[#providers + 1] = shared.new(resolved)
    end

    -- 4. Process credentials
    if resolved.credential_process then
        local process = require("aws.sdk.runtime.credentials.process")
